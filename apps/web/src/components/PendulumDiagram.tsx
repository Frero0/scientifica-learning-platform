  ...
  return (
    <div className="rounded-lg border border-ink/10 bg-ink p-5 text-white shadow-soft">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-white/10 px-3 py-1 text-sm text-white/75">T = 2*pi*sqrt(L/g)</span>
      </div>
      <div className="mt-6 grid gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
        <svg className="h-64 w-full" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
          ...
          <g className="pendulum-arm">
            <line stroke="rgba(101,211,188,0.9)" strokeLinecap="round" strokeWidth="4" x1="128" x2="164" y1="42" y2="178" />
        ...
        <div className="flex flex-col justify-center">
          <p className="text-base leading-7 text-white/72">{content.visualExplanation.description}</p>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            {content.visualExplanation.variables.map((variable) => (
              <div className="rounded-lg border border-white/10 bg-white/[0.06] p-3" key={variable.symbol}>
